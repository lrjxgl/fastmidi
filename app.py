...
class MIDIGeneratorUI:
    def __init__(self, config: Config, model_path: Optional[str] = None):
        ...
        if paddle.is_compiled_with_cuda():
            ...
            print(f'Using GPU: {paddle.device.cuda.device_count()} device(s) available')
        else:
    ...
    def generate_midi(self, emotion: str, style: str, key: str, mode: str,
                      ...
                      temperature: float, top_k: int, top_p: float) -> tuple:
        try:
            import random
            import time
            ...
            status_messages = []
            status_messages.append(f"🎵 开始生成音乐...")
            ...
            with paddle.no_grad():
                ...
                if num_tracks > 1:
                    ...
                    for i, (track_id, tokens) in enumerate(tokens_dict.items()):
                        ...
                        track_tokens[instrument_idx] = tokens.numpy().tolist()
                        ...
                        unknown = len(tokens) - single_notes - chord_starts - chord_note_tokens
                        ...
                        print(f"   - 音轨{i+1}: {len(tokens)} tokens (单音:{single_notes}, 和弦开始:{chord_starts}, 和弦音符:{chord_note_tokens}, 未知:{unknown})")
                    ...
                    print(f"   - 未知token: {all_unknown} ({all_unknown/total_tokens*100:.1f}%)")
            ...
            success = self.synthesizer.midi_to_wav(midi_path, wav_path)
            ...
            if success:
                ...
                beats_per_bar = 4
                target_duration = bars * beats_per_bar * (60.0 / bpm)
                print(f"   - 目标时长: {target_duration:.2f}秒 ({bars}小节 × {beats_per_bar}拍 × {60.0/bpm:.2f}秒/拍)")
                
                wav_final_path = os.path.join(self.output_dir, midi_filename.replace('.mid', '_final.wav'))
                try:
                    clip_cmd = [
                        'ffmpeg', '-y',
                        '-i', wav_path,
                    ...
                    if result.returncode == 0:
                        os.remove(wav_path)
                        os.rename(wav_final_path, wav_path)
                        print(f"   - 音频截取完成: {target_duration:.2f}秒")
                    else:
                        print(f"   - 音频截取警告: {result.stderr}")
                        wav_final_path = wav_path
                except FileNotFoundError:
                    print(f"   - ffmpeg未安装，跳过音频截取")
                    wav_final_path = wav_path
                except Exception as e:
                    print(f"   - 音频截取失败: {str(e)}")
                    wav_final_path = wav_path
                
                print(f"   - 最终文件路径: {wav_path}")
                ...
                status_messages.append(f"❌ WAV合成失败: {wav_time:.2f}秒")
    ...
    def create_interface(self):
        with gr.Blocks(title="Mamba MIDI 音乐生成器") as interface:
            gr.Markdown("# 🎵 Mamba MIDI 音乐生成器")
            gr.Markdown("基于Mamba架构的AI音乐生成模型，支持多种情绪、风格和参数配置")
    ...
    def launch(self, share: bool = False, server_port: int = 7864):
...
def main():
    ...
    print("="*60)
    print("Mamba MIDI 音乐生成器")
    print("="*60)
    print(f"\n设备: {paddle.get_device()}")
    print(f"输出目录: {os.path.join(config.PROCESSED_DATA_DIR, 'generated')}")
    
    model_path = os.path.join(config.CHECKPOINTS_DIR, 'best_model.pdparams')
